...
sed -i 's/tcp:6415:interface=127.0.0.1/tcp:2222:interface=0.0.0.0/' etc/cowrie.cfg\end{lstlisting}
...
The observed absence of attacks in AWS eu-north-1 underscores a critical limitation in contemporary honeypot deployment: strategic placement outweighs technical configuration. While our implementation rigorously applied Spitzner’s architectural principles, it could not overcome the inherent environmental sterility of a low-noise and well shielded cloud region. To solve this, we transition our infrastructure to a higher-friction zone, switching to OVH SoyouStart in Paris. This low-cost provider supplies unmonitored VPS in target-rich IP blocks with rather legacy equipments which consistently rank at the top of scan lists. Furthermore, OVH specifically authorizes the setup of a honeypot in its ToS (for research purposes, as long as there is no entrapment) and allows maximizing probe captures by disabling all firewalls. By retaining our same configurations and deception mechanisms but switching for a lower hanging fruit, we expect attackers to follow the path of least resistance and to be incited in paying our honeypot a visit, although our main priority remains data collection for attack profiling over total unique connections. 



\subsection{Credential Exploitation Patterns (RQ1)}
    % Top 10 username/password pairs from brute-force attacks

    % Comparison to Spitzner's 2002 findings (e.g., root:admin persistence)

    % Novel patterns: Cloud IAM roles, container-related usernames


\subsection{Persistence Mechanism Analysis}

% Case study: Attacker workflow from compromise to C2 setup


\subsection{Decoy Efficacy Metrics}


    
    % Engagement metrics:
    % Deception Tactic	Avg. Session Duration	Commands Executed
    % Baseline (no decoys)	2.1 min	4.2
    % Fake Filesystem	8.7 min	12.5
    % Cryptographic Lures	14.2 min	18.3

    % Attractor/Repellent artifacts (e.g., fake_credentials.txt vs. .honeypot_detection files)
