...
sudo nmap -sV -Pn -p 22,2222,610001 16.171.78.147
...
for i in {1..5}; do ssh -i ~/.ssh/wrong_key.pem ubuntu@16.171.78.147 -p 61001; done  
...
ssh -o PubkeyAuthentication=no -o PreferredAuthentications=password ubuntu@16.171.78.147 -p 61001  
\end{lstlisting}
...
sudo nmap -sV -Pn -p 22,2222 16.171.78.147 | grep -E '22/tcp|2222/tcp'  
\end{lstlisting}  
\
\begin{lstlisting}[language=bash,label={lst:cowrie-access},caption={Honeypot Engagement Testing}]  
# 2. Simulate attacker connection  
ssh -o StrictHostKeyChecking=no invalid_user@16.171.78.147 -p 22  
...
\section{Annex: Provoke }  
\label{annex:intel-gathering}  
\
\
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{doc//img/annex_d.png}
\end{figure}
\
